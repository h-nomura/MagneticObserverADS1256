...
def fig_plot3(df_print,labelList, title, fig_path, F_flag, dat_path = '', Yrange = 0):
    ...
    plt.savefig(fig_path)
    plt.close()
    return [title,value_2chmax,value_2chmaxAve,value_2chmin,value_2chminAve,value_5chmax,value_5chmaxAve,value_5chmin,value_5chminAve,value_8chmax,value_8chmaxAve,value_8chmin,value_8chminAve]
...
def Process(fileName,StartTime,EndTime, F_flag ,Yrange):
    ...
    if F_flag == "median2sig":
        ...
        data = fig_plot3(df_print,labelList,title, fig_path,F_flag,Yrange=int(Yrange))
        return data
        # Yrange = int(Yrange / 2)
        # my_makedirs(fig_dir + str(Yrange) + 'nt/')
        # fig_path = fig_dir + '/' + figFileDate + '_' + str(Yrange)+F_flag+siteInfo+'.png'
...
def main():
    ...
    data = [["title","max1","maxAve1","min1","minAve1","max2....."]]
    for i in range(85):
        # File_list1 = []
        # File_list2 = []
        # File_list3 = []
        ...
        try:
            data.append(Process([File_list1[i],File_list2[i],File_list3[i]],"00:00:00","23:59:59","median",125))
        except:
    ...
    with open('Sq.csv', 'w', newline="") as f:
        writer = csv.writer(f)
        for i in data:
            writer.writerow(i)
